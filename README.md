...
> *Your answer:* In the Chen notation we used the a diamond between the two entities with "N" and "M" on the lines. In the Crow's Foot notation we just have the "|{" symbol to show the "N" to "M" and with the builted svg there is only a straight line with symbols directly beside the entities.
...
> *Your answer:* With the changes our created svg would named "Library.svg" This would break the Workflow, because the workflow builds the "schema.puml" which wouldn't be updated anymore with a naming change.
...
> *Your answer:* PlantUML doesn't require to modeling the intermediate join table because it's abstracted away. When creating a relational model, then a join table should then be created.
...
>
> ![insert screenshot](assets/svg_Schema.png )
...
> *Your answer:* ```plantuml -tsvg schema.puml; echo $? ```
...
> *Your answer:* It isn't shown as an untracked file because we added this to the ".gitignore" and git doesn't check all files, which are in the ".gitignore".
...
>
> ![insert screenshot](assets/Commits.png)
...
> *Your answer:* The workflow triggers but failed because of the title. After doing and commiting minor changes the workflow runs as expected.
...
> *Your answer:* It also shows meta data for this tag with the name, message, object id and that there is no signature. The `-v`flag is the command to show the informations of this tag and could check if the signature of a user is valid. For this signature the tag needs to be created with the `-s` flag.
...
> *Your answer:*  If you would change the trigger to main, than the workflow wouldn't make sense anymore because it will be executed
> every time a commit to the main branch is made. A commit to the main branch doesn't need to be a new release, that could also be 
> bugfixes or minor changes. Also the naming of the Releases would be the same every time.
...
> *Your answer:* There is a github mechanism that the plantuml could be installed in a cache. With that you just install it once and 
> later on you safe those 20-30 seconds of installation.
...
>
> [Workflow Run](assets/Workflow.png)
...
> *Your answer:* The installing step takes 12s and the render step just 2s. I think 
> the installing takes longer because it needs to download and setup the PlanUML and we only build a very small svg.
> The optimisation should be directed to the installing, for example to only install it once in the cache.
...
> *Your answer:* Both svg are exactly the same. This is a good sign that it's reproducible and doesn't need
> to have any specific settings.
...
> *Your answer:* When we store it as a PNG than we would only see that something changed, 
> but not what exactly in this file was changed.
...
> *Your answer:* If there are merge conflicts in a binary file, you don't have the opportunity to 
> resolve them. One of both needs to do his changes afterwards again.
...
> *Your answer:* A created tag always shows to the same commit. When you create a branch for
> a release, than you could still commit to this branch and change the release. This could
> be beneficial when you have a customer demo and need to do some live changes for the presentation.
...
> *Your answer:* The CI pipeline eliminates the manual work to build the image.
> So it isn't required to install the PlantUML and guarantees, that the image
> match the source.
...
>
> ![Terminal and svg](assets/Reflektion.png)
